(function () {
  ...
  const attendanceMethods = [
    ...
    },
    {
      icon: "map",
      tagAr: "نطاق جغرافي",
      tagEn: "Geofence layer",
      titleAr: "تحقق الموقع",
      titleEn: "Geofencing",
      descAr: "يضيف طبقة تحقق مكانية للتأكد من أن الحضور يتم داخل النطاق المعتمد للمحاضرة أو البوابة.",
      descEn: "Adds a location verification layer to confirm attendance occurs inside the approved lecture or gate area.",
      chipsAr: ["مكاني", "خصوصية"],
      chipsEn: ["Location-aware", "Privacy-aware"],
    },
  ];

  const stats = [
    { value: "5", labelAr: "قنوات حضور", labelEn: "Attendance Channels" },
  ...
  function renderAttendanceMethods() {
    ...
    grid.innerHTML = attendanceMethods
      ...
      <article class="feature-card feature-card--method feature-card--method-${m.icon} reveal${extra}" role="button" tabindex="0" aria-expanded="false" data-method-idx="${idx}" data-tilt-card>
        <div class="method-signal method-signal--${m.icon}" aria-hidden="true"><span></span><span></span><span></span></div>
  ...
  function renderGate() {
    ...
    const gateCard = panel.closest(".gate-left");
    if (gateCard && !gateCard.querySelector(".gate-flow-visual")) {
      prog.parentElement?.insertAdjacentHTML(
        "afterend",
        `
        <div class="gate-flow-visual" aria-hidden="true">
          <span class="gate-flow-node gate-flow-node--card"></span>
          <span class="gate-flow-beam"></span>
          <span class="gate-flow-node gate-flow-node--scan"></span>
          <span class="gate-flow-node gate-flow-node--ok"></span>
        </div>`
      );
    }
  ...
  function initSafeParallax() {
    const reduced = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced) return;

    const sections = Array.from(
      document.querySelectorAll(".section-features, .section-workflow, .section-gate, .section-stack, .section-compare, .section-live-dashboard, .section-mini-demo")
    );
    ...
    const update = () => {
      sections.forEach((section) => {
        ...
        const shift = Math.max(-46, Math.min(46, midpoint * -0.045));
        section.style.setProperty("--parallax-shift", `${shift.toFixed(1)}px`);
      });
      ticking = false;
    };
  ...
  function initWorkflowAnimation() {
    const shell = document.getElementById("attendanceWorkflow");
    if (!shell) return;
    const steps = Array.from(shell.querySelectorAll("[data-flow-step]"));
    ...
    const setActive = (idx) => {
      active = ((idx % steps.length) + steps.length) % steps.length;
      steps.forEach((step, stepIdx) => {
        step.classList.toggle("active", stepIdx === active);
        step.classList.toggle("done", stepIdx < active);
      });
      shell.style.setProperty("--workflow-progress", `${(active / (steps.length - 1)) * 100}%`);
    };
    ...
    const start = () => {
      if (timer) return;
      timer = window.setInterval(() => setActive(active + 1), 2200);
    };
    const stop = () => {
      if (!timer) return;
  ...
  function initGateScene() {
    const scene = document.getElementById("femaleSecurityScene");
    if (!scene) return;
    const states = Array.from(scene.querySelectorAll("[data-gate-state]"));
    ...
    const setState = (next) => {
      ...
      states.forEach((state, stateIdx) => state.classList.toggle("active", stateIdx === idx));
      scene.style.setProperty("--security-state", String(idx));
    };

    states.forEach((state, stateIdx) => {
      state.addEventListener("click", () => setState(stateIdx));
      state.tabIndex = 0;
    ...
    const start = () => {
      if (timer) return;
      timer = window.setInterval(() => setState(idx + 1), 3600);
    };
    const stop = () => {
      if (!timer) return;
      window.clearInterval(timer);
  ...
  function initVerificationStatusStrip() {
    const strip = document.getElementById("verificationStatusStrip");
    if (!strip) return;
    const events = Array.from(strip.querySelectorAll("[data-status-event]"));
    if (!events.length) return;

    let active = 0;
    const reduced = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const setActive = (idx) => {
      active = ((idx % events.length) + events.length) % events.length;
      events.forEach((event, eventIdx) => event.classList.toggle("active", eventIdx === active));
    };

    events.forEach((event, idx) => {
      event.addEventListener("click", () => setActive(idx));
      event.tabIndex = 0;
  ...
  function initLiveDashboard() {
    ...
    if (!dashboard) return;
    initDashboardCounters(dashboard.querySelectorAll("[data-stat-value]"));
  }

  function initRoleJourney() {
    const shell = document.getElementById("roleJourney");
    if (!shell) return;
    const tabs = Array.from(shell.querySelectorAll("[data-role-tab]"));
    const panels = Array.from(shell.querySelectorAll("[data-role-panel]"));
    if (!tabs.length || !panels.length) return;

    const activate = (role) => {
      tabs.forEach((tab) => {
        const on = tab.getAttribute("data-role-tab") === role;
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => activate(tab.getAttribute("data-role-tab") || "student"));
      tab.addEventListener("keydown", (e) => {
        ...
        const next = tabs[(idx + (rtl ? -dir : dir) + tabs.length) % tabs.length];
        next.focus();
        activate(next.getAttribute("data-role-tab") || "student");
      });
    });
  }

  function initSmartRadar() {
    const radar = document.getElementById("smartVerificationRadar");
    if (!radar) return;
    const methods = Array.from(radar.querySelectorAll("[data-radar-method]"));
    if (!methods.length) return;

    let active = 0;
    const reduced = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const activate = (idx) => {
      active = ((idx % methods.length) + methods.length) % methods.length;
      methods.forEach((method, methodIdx) => method.classList.toggle("active", methodIdx === active));
      radar.classList.toggle("verified", active === methods.length - 1);
    };

    methods.forEach((method, idx) => {
      method.addEventListener("click", () => activate(idx));
    });
    ...
    const start = () => {
      if (timer) return;
      timer = window.setInterval(() => activate(active + 1), 2400);
    };
    const stop = () => {
      if (!timer) return;
      window.clearInterval(timer);
  ...
  function initCampusMode() {
    const mode = document.getElementById("campusMode");
    if (!mode) return;
    const events = Array.from(mode.querySelectorAll("[data-campus-event]"));
    if (!events.length) return;
    ...
    events.forEach((event, idx) => {
      event.addEventListener("click", () => activate(idx));
      event.tabIndex = 0;
      event.setAttribute("role", "button");
      event.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
    ...
    const start = () => {
      if (timer) return;
      timer = window.setInterval(() => activate(active + 1), 3000);
    };
    const stop = () => {
      if (!timer) return;
      window.clearInterval(timer);
  ...
  function initImpactHighlights() {
    const impact = document.getElementById("impactHighlights");
    if (!impact) return;
    initDashboardCounters(impact.querySelectorAll("[data-stat-value]"));
  }

  function initMiniDemo() {
    const demo = document.getElementById("attendanceDemo");
    const btn = document.getElementById("demoRun");
    if (!demo || !btn) return;

    const steps = Array.from(demo.querySelectorAll("[data-demo-step]"));
    const qrGrid = document.getElementById("demoQrGrid");
    const code = document.getElementById("demoTokenCode");
    const report = document.getElementById("demoReportCount");
    const reportStatus = document.getElementById("demoReportStatus");
    let running = false;
    let reportValue = Number(report?.textContent || 86);

    if (qrGrid && !qrGrid.childElementCount) {
      for (let i = 0; i < 49; i += 1) {
        const cell = document.createElement("span");
        cell.className = (i * 7 + i) % 5 < 2 ? "on" : "";
    ...
    const setStep = (idx) => {
      ...
      demo.style.setProperty("--demo-progress", `${(idx / (steps.length - 1)) * 100}%`);
    };
    ...
    btn.addEventListener("click", async () => {
      ...
      running = true;
      btn.disabled = true;
      btn.textContent = t("جاري التنفيذ...", "Running...");
      demo.classList.add("running");
      demo.classList.remove("recorded");
      if (reportStatus) {
        reportStatus.setAttribute("data-ar", "قيد التحقق...");
        reportStatus.setAttribute("data-en", "Verification in progress...");
        reportStatus.textContent = t("قيد التحقق...", "Verification in progress...");
      }
      setStep(0);

      const token = `MH-${Math.floor(1000 + Math.random() * 9000)}`;
      if (code) code.textContent = token;

      for (let i = 1; i < steps.length; i += 1) {
        setStep(i);
        await wait(i === steps.length - 1 ? 500 : 850);
      }

      reportValue += 1;
      if (report) report.textContent = String(reportValue);
      demo.classList.add("recorded");
      if (reportStatus) {
        reportStatus.setAttribute("data-ar", "تم تحديث التقرير");
        reportStatus.setAttribute("data-en", "Report Updated");
        reportStatus.textContent = t("تم تحديث التقرير", "Report Updated");
      }
      btn.textContent = t("إعادة التجربة", "Run Again");
      btn.disabled = false;
      running = false;
      window.setTimeout(() => {
