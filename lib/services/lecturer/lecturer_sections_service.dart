...
class LecturerSectionsService {
  ...
  Future<Map<String, String>> _fetchCourseDisplayNames(Set<String> courseCodes) async {
    final result = <String, String>{};
    if (courseCodes.isEmpty) return result;

    final isArabic = LecturerLanguageController.isArabic;
    for (final code in courseCodes) {
      final doc = await _firestore.collection(_coursesCollection).doc(code).get();
      if (!doc.exists || doc.data() == null) continue;
      final d = doc.data()!;
      // Firestore: courseName_Ar للعربي، courseName للإنجليزي
      final nameAr = (d['courseName_Ar'] ?? d['courseNameAr'] ?? d['nameAr'] ?? '').toString().trim();
      final nameEn = (d['courseName'] ?? d['courseNameEn'] ?? d['nameEn'] ?? '').toString().trim();
      final fallback = nameAr.isNotEmpty ? nameAr : nameEn;
      final display = isArabic
          ? (nameAr.isNotEmpty ? nameAr : (nameEn.isNotEmpty ? nameEn : fallback))
          : (nameEn.isNotEmpty ? nameEn : (nameAr.isNotEmpty ? nameAr : fallback));
  ...
  List<LectureItem> _sectionToLectureItems(String sectionDocId, Map<String, dynamic> data, [String? courseDisplayName]) {
    final schedule = data['schedule'] as List<dynamic>?;
    if (schedule == null || schedule.isEmpty) return [];

    final courseCode = (data['courseCode'] ?? '').toString().trim();
    final courseName = (courseDisplayName != null && courseDisplayName.isNotEmpty)
        ? courseDisplayName
