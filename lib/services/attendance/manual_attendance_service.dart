...
class ManualAttendanceService {
  ...
  static const String explicitSessionOpenedField = 'explicitSessionOpened';
  ...
  Future<List<ManualAttendanceSession>> getSessionsForSectionIds(
    ...
    for (final snap in snapshots) {
      for (final doc in snap.docs) {
        final data = doc.data();
        if (openedOnly && !_isOpenedManualSessionData(data)) {
          continue;
        }
        sessions.add(ManualAttendanceSession.fromDoc(doc));
      }
    }
    sessions.sort((a, b) {
      final byDate = b.lectureDate.compareTo(a.lectureDate);
    ...
    for (final batch in _chunk(ids, finalizeBatchSize)) {
      await Future.wait(
        batch.map((id) async {
          ...
          if (data == null || !_isExplicitlyOpenedSessionData(data)) {
            return;
          }
  ...
  Future<String?> ensureDefaultPresentSessionForEndedLecture({
    ...
    if (existingSnap.exists) {
      ...
      if (_isExplicitlyOpenedSessionData(data)) {
        return sessionId;
      }
      await _markSessionAsDefaultPresent(
        sessionId: sessionId,
        sectionId: sectionId,
        lecture: lecture,
        date: date,
  ...
  Future<bool> finalizeSessionPendingAsAbsent(
    ...
    if (!_isExplicitlyOpenedSessionData(sessionData)) {
      return false;
    }
  ...
  Future<void> _upsertSessionDoc({
    ...
    final lecturerId =
        LecturerAuthService.instance.currentLecturer?.lecturerId.trim() ?? '';
    final convertingFromDefaultPresent =
    ...
    final payload = <String, dynamic>{
      ...
      explicitSessionOpenedField: true,
      'explicitSessionOpenedAt': Timestamp.fromDate(sessionOpenedAt),
      'autoGenerated': false,
    ...
    if (lecturerId.isNotEmpty) {
      ...
      payload['explicitSessionOpenedBy'] = lecturerId;
    }
  ...
  static bool _isExplicitlyOpenedSessionData(Map<String, dynamic> data) {
    return data[explicitSessionOpenedField] == true;
  }
  ...
  Future<void> _createDefaultPresentSessionWithRecords({
    ...
    await sessionRef.set(<String, dynamic>{
      ...
      explicitSessionOpenedField: false,
      'autoGenerated': true,
    ...
    await _ensureDefaultPresentRecordsForSession(
      ...
      forcePresentExisting: false,
    );
  }

  Future<void> _markSessionAsDefaultPresent({
    required String sessionId,
    required String sectionId,
    required LectureItem lecture,
    required DateTime date,
    ...
    await _firestore
        ...
        .set(<String, dynamic>{
          ...
          'createdBy': createdBySystem,
          'attendanceFinalized': true,
          'sessionOpenedAt': FieldValue.delete(),
          'updatedAt': FieldValue.serverTimestamp(),
          ...termPayload,
        }, SetOptions(merge: true));
  }
  ...
  Future<void> _ensureDefaultPresentRecordsForSession({
    ...
    required bool forcePresentExisting,
  }) async {
    if (_isExplicitlyOpenedSessionData(existingSessionData)) {
      return;
    }
  ...
  Future<List<ManualAttendanceSession>> getCountableSessionsForSection(
    ...
    for (final doc in snapshot.docs) {
      if (!_isExplicitlyOpenedSessionData(doc.data())) {
        continue;
      }
